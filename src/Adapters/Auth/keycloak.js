...
const handleAuth = async ({ access_token, id, roles, groups, is } = {}, { config } = {}) => {
  if (!(access_token && id)) {
  ...
  try {
    console.log('hehe')
    console.log(config['auth-server-url'])
    console.log("ooo")
    console.log(`/realms/${config['realm']}/protocol/openid-connect/userinfo`)
    const response = await httpsRequest.get({
      hostName: config['auth-server-url'],
